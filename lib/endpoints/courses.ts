...
export async function getCourseSections<T = unknown>(courseSlug: string, schema?: Schema<T>): Promise<T> {
  return fetchJson<T>({ path: `/api/v1/courses/${courseSlug}/sections`, schema })
}

export async function updateCourse<T = unknown>(courseId: number | string, body: CourseUpdatePayload, schema?: Schema<T>): Promise<T> {
  return fetchJson<T>({ path: `/api/v1/courses/${courseId}` , method: "PUT", body, schema })
}

export async function createCourse<T = unknown>(body: CourseCreatePayload, schema?: Schema<T>): Promise<T> {
